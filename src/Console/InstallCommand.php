...
class InstallCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        if (config('twill-ai.mcp.enabled')) {
            $this->line('  5. MCP is enabled: run php artisan passport:keys, then php artisan mcp:client-create');
            $this->line('     to authorise a connector. php artisan mcp:doctor checks the whole sign-in chain.');
        } else {
            $this->newLine();
